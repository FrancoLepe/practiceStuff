...
function App() {
  ...
  const problems = questions.map((question) => (
    <div key={question.id}>
      ...
      {question.picture && (
        <img src={question.picture} alt={question.picName} />
      )}
    </div>
  ));
  ...
  return (
    <div className={active ? "App" : "App.darkmode"}>
      <Navbar active={active} setActive={setActive} handleClick={handleClick} />
      <h1> Welcome</h1>

      <button className="yoLegs" onClick={handleYoLegsButton}>
        "yo legs" Button
      </button>
